	...
	private function require_provider_key( $provider ) {
		...
		if ( false === $key ) {
			...
			return $this->error_response(
				...
				sprintf(
					/* translators: %s: Provider name */
					__( '%s API key not configured. Go to MCPWP > Integrations to add it.', 'site-pilot-ai' ),
					$name
				),
				400
			);
		...
		if ( ! $preferred ) {
			return $this->error_response(
				...
				sprintf(
					...
					__( 'No AI provider configured for %s. Go to MCPWP > Integrations to add one.', 'site-pilot-ai' ),
					$capability
				),
				400
			);
