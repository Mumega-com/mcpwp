	...
	private static function action_for_issue( $issue ) {
		...
		return array_merge(
			...
			array(
				...
				'can_auto_prepare' => false,
				'next_step'        => __( 'Review this issue manually and choose the matching approval-safe playbook.', 'site-pilot-ai' ),
				'reason'           => __( 'No deterministic autofix strategy exists for this issue code yet.', 'site-pilot-ai' ),
			)
		);
	}
	...
	private static function strategies() {
		return array(
			'missing_meta_description' => array(
				...
				'next_step'        => __( 'Draft a concise meta description from visible page content, then request human approval before writing SEO metadata.', 'site-pilot-ai' ),
				'reason'           => __( 'SEO metadata changes affect search snippets and must remain approval-gated.', 'site-pilot-ai' ),
			),
			'meta_description_length' => array(
				...
				'targets'          => array( '_yoast_wpseo_metadesc', 'rank_math_description', '_aioseo_description' ),
				'can_auto_prepare' => true,
				'next_step'        => __( 'Prepare a replacement meta description that preserves the page promise and ask for approval before writing it.', 'site-pilot-ai' ),
				'reason'           => __( 'Snippet copy is search-facing and should be approved before mutation.', 'site-pilot-ai' ),
			),
			'missing_image_alt' => array(
				...
				'playbook'         => 'seo_audit_triage',
				'can_auto_prepare' => true,
				'next_step'        => __( 'Inspect the image in page context, draft factual alt text, and require human approval before media metadata changes.', 'site-pilot-ai' ),
				'reason'           => __( 'Alt text must describe the actual image and visible context, not guessed keywords.', 'site-pilot-ai' ),
			),
			'no_internal_outbound_links' => array(
				...
				'can_auto_prepare' => true,
				'next_step'        => __( 'Suggest a relevant internal link from the content graph, then use approval-first link insertion.', 'site-pilot-ai' ),
				'reason'           => __( 'Internal links change editorial meaning and should be added through approval-first content edits.', 'site-pilot-ai' ),
			),
			'orphan_content' => array(
				...
				'playbook'         => 'internal_link_improvement',
				'can_auto_prepare' => true,
				'next_step'        => __( 'Find relevant source pages in the content graph and propose inbound links through approval-first edits.', 'site-pilot-ai' ),
				'reason'           => __( 'Orphan fixes require editorially relevant links, not automatic keyword stuffing.', 'site-pilot-ai' ),
			),
			'thin_content' => array(
				...
				'can_auto_prepare' => true,
				'next_step'        => __( 'Prepare one Gutenberg section that adds useful, sourced substance and submit it as an approval request.', 'site-pilot-ai' ),
				'reason'           => __( 'Content expansion affects page claims and must be reviewed before publication.', 'site-pilot-ai' ),
			),
			'missing_h1' => array(
				'strategy'         => 'content_patch',
				'tool'             => 'wp_patch_block_section',
				'playbook'         => 'update_gutenberg_section',
				'can_auto_prepare' => true,
				'next_step'        => __( 'Prepare a native Heading block that matches the visible page topic and submit an approval request.', 'site-pilot-ai' ),
				'reason'           => __( 'Primary headings are user-visible and must match the page intent.', 'site-pilot-ai' ),
			),
			'heading_order_skip' => array(
				...
				'playbook'         => 'update_gutenberg_section',
				'can_auto_prepare' => true,
				'next_step'        => __( 'Prepare the smallest heading-level correction in native blocks and submit it for approval.', 'site-pilot-ai' ),
				'reason'           => __( 'Heading hierarchy changes visible structure and accessibility semantics.', 'site-pilot-ai' ),
			),
			'multiple_h1' => array(
				'strategy'         => 'content_patch',
				'tool'             => 'wp_patch_block_section',
				'playbook'         => 'update_gutenberg_section',
				'can_auto_prepare' => true,
				'next_step'        => __( 'Prepare a minimal heading hierarchy correction and submit it for approval.', 'site-pilot-ai' ),
				'reason'           => __( 'Heading hierarchy changes visible structure and accessibility semantics.', 'site-pilot-ai' ),
			),
			'schema_hint_missing' => array(
				...
				'playbook'         => 'seo_audit_triage',
				'can_auto_prepare' => false,
				'next_step'        => __( 'Validate visible content and choose schema only when the page supports it.', 'site-pilot-ai' ),
				'reason'           => __( 'Schema markup can create search-facing claims and should not be inferred automatically.', 'site-pilot-ai' ),
			),
		);
	}
