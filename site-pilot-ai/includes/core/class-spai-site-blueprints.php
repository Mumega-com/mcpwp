 ...
 */
class Spai_Site_Blueprints {

	const OPTION_KEY = 'spai_site_blueprints';
	...
	public static function deploy( string $id, array $overrides = array() ): array {
		...
		foreach ( $blueprint['pages'] as $page_def ) {
			...
			$page_id = wp_insert_post( array(
				...
				'meta_input'   => array(
					'_spai_blueprint_id'       => $id,
					'_spai_blueprint_sections' => wp_json_encode( $page_def['sections'] ?? array() ),
				),
			) );
		...
		if ( ! empty( $blueprint['site_context'] ) && empty( get_option( 'spai_site_context', '' ) ) ) {
			update_option( 'spai_site_context', wp_kses_post( $blueprint['site_context'] ), false );
		}
