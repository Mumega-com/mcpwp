...
**Rationale:** Per-site annual licensing is the WordPress developer standard (cf. Gravity Forms $59, Elementor Pro $99). Pro at $79 is below Elementor Pro and above basic utility plugins — right for the category. Agency tier unlocks when the agency dashboard ships (Sprint next quarter).
...
- [ ] Update free/wporg branch to v2.8.31 from main via build-wporg.sh
- [ ] Run Plugin Check (target: 0 ERRORs, minimize WARNINGs)
- [ ] Verify screenshots are current (4 exist in assets/)
- [ ] Submit to WordPress.org plugin repository
- [ ] Set up SVN assets (banners, icons) separately from plugin ZIP

### Phase 3 — Freemius Paid Launch
- [ ] Configure Pro plan ($79/year/site) in Freemius dashboard
- [ ] Configure Agency plan ($249/year/unlimited) — hold until agency dashboard ships
- [ ] Enable checkout on mcpwp.net
- [ ] Verify upgrade path from WP.org free install to Pro
- [ ] Test Freemius trial flow
...
- [ ] Outreach to MCP-adjacent communities: Claude users, Cursor Discord

---

## Hero Use Case (for website / Product Hunt)
...
1. **`class-spai-rest-site.php` still 4,506 lines** — further extraction possible (onboard method ~260 lines is a candidate). Acceptable for launch but track.
2. **Capability registry not fully implemented** — `CAPABILITY_MAP.md` defines the target; actual enforcement is scattered. Should be centralized before Agency tier ships.
3. **REST namespace `site-pilot-ai/v1`** — visible to API consumers, changing it is breaking. Leave for v3.0 major bump only.
4. **Internal `SPAI_` prefix** — invisible to users, renaming would break DB options. Do not rename.
5. **`free/wporg` branch at v2.8.6** — must be updated as Phase 2 first step.
