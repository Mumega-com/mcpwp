...
- For internal links, call `wp_suggest_internal_links` first. Use only returned graph URLs and apply accepted links through `wp_patch_block_section` or `wp_set_blocks` with approval enabled.
