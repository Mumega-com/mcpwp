...
- WP.org Plugin Check baseline: `0 ERROR`, `402 WARNING` on packaged `mcpwp` ZIP for `2.8.30`.
...
- Freemius activation smoke: passed on local WordPress using isolated `mcpwp-freemius-test` folder; `MCPWP_FREEMIUS_BUILD=true`, `mcpwp_fs()` exists, Pro bootstrap present, and Freemius update-refresh hook updates `mcpwp_last_update_flush` from the plugins page. WP-CLI activation no longer emits the Freemius SDK `HTTP_HOST` warning.
...
- Text domain normalized to `mcpwp`.
- WP.org package root changed to `mcpwp/`.
- Canonical site URLs updated to `https://mcpwp.net/`.
- Freemius build script added.
- Freemius bootstrap gated behind `MCPWP_FREEMIUS_BUILD`.
- Freemius license state recognized by `Mcpwp_License`.
- WP.org build excludes Freemius SDK and updater.
- WP.org build excludes `includes/pro/` and disables Pro MCP exposure through `MCPWP_WPORG_BUILD`.
- `mcpwp.php` only loads Pro modules outside WP.org builds when `Mcpwp_License::is_pro()` is true.
...
Recent bus messages were sent to `mcpwp` with findings about PR #257 and the packaging validation.

Branding note: public product/display name is now `MCPWP`. Keep the WP.org slug/text domain as `mcpwp`; keep legacy route/package identifiers such as `mcpwp/v1` unless a migration is planned.
...
AI-first hooks progress: first slice adds `Mcpwp_Event_Store`, recent normalized event history, `mcpwp_event_emitted`, specific hooks such as `mcpwp_approval_created` and `mcpwp_seo_audit_completed`, webhook forwarding through existing signed webhooks, REST `/events/schema`, REST `/events`, MCP `wp_get_event_schema`, and MCP `wp_list_mcp_events`.
...
Site-state snapshot progress: #313 first slice adds `Mcpwp_Site_State`, REST `GET /mcpwp/v1/site-state`, and MCP `wp_get_site_state`. The snapshot summarizes site identity, context configuration, content counts, graph health, approval queues, stored SEO issues, recent normalized events, capability flags, and recommended next actions so agents can start from one coherent state model.
...
Deterministic playbook progress: #315 first slice adds `Mcpwp_Agent_Playbooks`, REST `GET /mcpwp/v1/agent-playbooks`, and MCP `wp_get_agent_playbook`. Current contracts cover `build_gutenberg_page`, `update_gutenberg_section`, `seo_audit_triage`, `internal_link_improvement`, and `rollback_change` with required tools, validation gates, approval gates, rollback paths, and stop conditions.

Content coherence progress: #316 first slice adds `Mcpwp_Content_Coherence`, REST `GET /mcpwp/v1/content-coherence`, and MCP `wp_get_content_coherence_report`. The score uses site-state inputs across context, graph connection, content depth/freshness, stored SEO issues, approval risk, and event risk, then maps recommendations to deterministic playbooks.

SEO autofix progress: #307 first slice adds `Mcpwp_SEO_Autofix`, REST `GET /mcpwp/v1/seo/autofix-plan`, and MCP `wp_run_seo_autofix_plan`. It is intentionally read-only: it maps stored open SEO issues to deterministic strategies, tools, playbooks, approval requirements, and manual-review boundaries. `can_auto_apply` is false for every action until a dedicated approval adapter exists for the target mutation type.

Search performance progress: #304 first slice adds `Mcpwp_Search_Performance`, REST `POST /mcpwp/v1/seo/search-performance/import`, REST `GET /mcpwp/v1/seo/search-performance`, MCP `wp_import_search_performance`, and MCP `wp_get_seo_trends`. It stores explicit Search Console, Bing, rank tracker, or manual rows and reports top queries, top URLs, daily aggregates, CTR, average position, provider mix, and import history without external API fetching.

WooCommerce SEO progress: #311 first slice adds `Mcpwp_WooCommerce_SEO`, REST `GET /mcpwp/v1/seo/woocommerce`, and MCP `wp_get_woocommerce_seo_report`. It is read-only and reports product content depth, short descriptions, category/image evidence, SKU/price/stock signals, imported search performance, and approval-safe next steps.

Coming sprint sequence: #312 event hooks, #313 site-state snapshot, #314 Control Room event inbox, #315 deterministic playbooks, #316 content coherence score, #307 SEO autofix, #304 Search Console/Bing imports, and #311 WooCommerce SEO intelligence are implemented as first slices. Next backlog should choose deeper provider connectors, product category drill-downs, approved mutation adapters, or keyword/topic inventory.

Implementation progress on PR #277: block safety first slice now exposes `wp_validate_blocks` and `POST /mcpwp/v1/blocks/validate`, adds safety reports to parse/serialize responses, and makes `wp_set_blocks` reject classic HTML, `core/html`, inline script/style tags, and unsafe iframes by default unless an explicit approval note is supplied. Internal graph first slice now exposes `wp_get_content_graph` and `GET /mcpwp/v1/content-graph` with nodes, content links, parent/child edges, inbound/outbound counts, anchors, headings, menu presence, and orphan candidates.
...
Section patching progress: `wp_patch_block_section` and `POST /mcpwp/v1/blocks/{id}/section` now replace one selected Gutenberg section by path, anchor, or heading. The endpoint validates replacement markup and creates an approval request by default; immediate saves require `approval_required=false`.

Internal graph progress: `wp_suggest_internal_links` and `GET /mcpwp/v1/content-graph/suggestions` now return read-only internal link suggestions from the content graph. Suggestions use existing graph URLs only, include a conservative anchor and approval diff, and do not mutate content. `wp_apply_internal_link` and `POST /mcpwp/v1/content-graph/apply-link` now apply accepted graph targets by creating an approval request by default and appending a native Gutenberg related-link paragraph.

Internal link validation progress: `wp_validate_internal_links` and `GET /mcpwp/v1/content-graph/validate-links` now report self-links, duplicate internal targets, empty/weak anchors, missing targets, unpublished targets, and non-canonical URLs without mutating content.
...
SEO readiness progress: `wp_validate_seo_readiness` and `GET /mcpwp/v1/seo/readiness/{id}` now provide a read-only pre-publish check for title, slug, content depth, H1, heading order, meta description, image alt text, internal links, orphan state, noindex, canonical override, robots.txt, sitemap hint, and schema hint.

Structured data progress: `wp_validate_structured_data` and `GET /mcpwp/v1/seo/structured-data/{id}` now inventory JSON-LD, microdata, and schema.org hints; report invalid JSON-LD, missing `@context`/`@type`, basic Article/FAQ/Product shape issues, and page-appropriate schema recommendations without mutating content.

Media SEO progress: `wp_audit_media_seo` and `GET /mcpwp/v1/seo/media/{id}` now inspect featured images and content images, reporting missing alt text, generic filenames, large local files, missing dimensions, missing lazy-loading hints, duplicate image reuse, and external image counts without mutating media.

Site SEO audit progress: `wp_seo_audit_site` and `GET /mcpwp/v1/seo/audit` now aggregate readiness, structured data, and media SEO issues across recent posts/pages into prioritized URL rows, category counts, and top issue codes without mutating content.

Content quality progress: `wp_audit_content_quality` and `GET /mcpwp/v1/seo/content-quality/{id}` now report word depth, summary intro, question coverage, entity-like names, freshness, trust signals, and external reference hints for AI-search/citation readiness without mutating content. `wp_seo_audit_site` includes this category in aggregate counts.

Stored SEO issue progress: `wp_seo_audit_site` and `GET /mcpwp/v1/seo/audit` now accept `store=true` to persist recent audit runs and normalized top issue records. `wp_get_seo_issues` and `GET /mcpwp/v1/seo/issues` list issues by status, severity, category, post ID, run ID, and limit. Resolution is scoped to posts included in the current run so partial audits do not incorrectly close issues from skipped content.
