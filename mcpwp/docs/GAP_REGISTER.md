...
Status: first read-only implementation added `wp_get_content_graph` and `GET /mcpwp/v1/content-graph`. It returns content nodes, content-link edges, parent/child edges, inbound/outbound counts, menu presence, headings, anchors, and orphan candidates. Link workflows now include `wp_suggest_internal_links` for graph-based suggestions, `wp_apply_internal_link` for approval-first insertion of existing graph targets, and `wp_validate_internal_links` for read-only validation. Weighted graph signals now include taxonomy edges, menu depth, freshness score, hub score, orphan severity, and PageRank-style rank score. Richer contextual placement is still open.
...
Status: first pre-publish slice adds `wp_validate_seo_readiness` and `GET /mcpwp/v1/seo/readiness/{id}`. It checks title, slug, thin content, H1, heading order, meta description, image alt text, internal links, orphan state, noindex, canonical overrides, robots.txt, sitemap hints, and schema hints without mutating content. Structured data first slice adds `wp_validate_structured_data` and `GET /mcpwp/v1/seo/structured-data/{id}`. Media SEO first slice adds `wp_audit_media_seo` and `GET /mcpwp/v1/seo/media/{id}`. Site audit first slice adds `wp_seo_audit_site` and `GET /mcpwp/v1/seo/audit`. Content quality first slice adds `wp_audit_content_quality` and `GET /mcpwp/v1/seo/content-quality/{id}`.

Stored issue status: `wp_seo_audit_site(store=true)` now stores recent audit runs and normalized top issue records. `wp_get_seo_issues` and `GET /mcpwp/v1/seo/issues` expose status, severity, category, post, run, and limit filters. This is the first lightweight issue model; scheduled crawls, trends, dashboard UI, and richer evidence remain open.
...
- #316 Add content coherence scoring and recommendations. Status: first implementation added `wp_get_content_coherence_report` and REST `GET /mcpwp/v1/content-coherence`.
...
6. #307 Approval-safe SEO autofix workflows. Status: first read-only planner added `wp_run_seo_autofix_plan` and REST `GET /mcpwp/v1/seo/autofix-plan`; no direct mutation is allowed.
...
8. #311 WooCommerce SEO intelligence. Status: first read-only product SEO report added `wp_get_woocommerce_seo_report` and REST `GET /mcpwp/v1/seo/woocommerce`; product mutation adapters and category drill-downs remain open.
...
Status: first implementation added `wp_get_agent_playbook` and REST `GET /mcpwp/v1/agent-playbooks` with deterministic contracts for Gutenberg page creation, one-section updates, SEO audit triage, internal link improvements, and rollback.
