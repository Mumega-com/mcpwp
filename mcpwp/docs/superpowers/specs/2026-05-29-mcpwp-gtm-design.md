...
|---------|-----|----------|
| **npm** (`mcpwp`) | Publish, rename from `mcpwp` | Immediate |
...
- [ ] Rename npm package `mcpwp` → `mcpwp`, publish
...
- [ ] Run Plugin Check on `mcpwp-2.8.31.zip` (target: 0 ERRORs)
...
**What MCPWP does about it:** One MCPWP connection supports multiple sites. The config file (`~/.mcpwp/config.json`) holds all client credentials. The agent calls `wp_switch_site("client-name")` to change context. No adding/removing MCPs. No client data mixing.

```json
{
  "sites": {
    "acme-corp":    { "url": "https://acme.com",    "apiKey": "mcpwp_..." },
    "client-b":     { "url": "https://clientb.com", "apiKey": "mcpwp_..." },
    "client-c":     { "url": "https://clientc.com", "apiKey": "mcpwp_..." }
  },
...
1. `class-mcpwp-rest-site.php` still 4,506 lines — further split possible, non-blocking
2. Capability registry scattered — centralize before Agency tier
3. REST namespace `mcpwp/v1` — leave for v3 major bump
4. Internal `MCPWP_` prefix — do not rename (breaks DB options on existing installs)
