...
#307 status: first implementation adds `Mcpwp_SEO_Autofix`, REST `GET /mcpwp/v1/seo/autofix-plan`, and MCP `wp_run_seo_autofix_plan`. The planner consumes stored open SEO issues and returns strategy, tool, playbook, next step, approval requirement, and guardrails. It deliberately never applies fixes directly; every action reports `can_auto_apply=false`.

#304 status: first implementation adds `Mcpwp_Search_Performance`, REST `POST /mcpwp/v1/seo/search-performance/import`, REST `GET /mcpwp/v1/seo/search-performance`, MCP `wp_import_search_performance`, and MCP `wp_get_seo_trends`. This slice stores explicit Search Console/Bing/manual exports and reports top queries, top URLs, daily aggregates, provider mix, CTR, and average position. It does not fetch external APIs yet.

#311 status: first implementation adds `Mcpwp_WooCommerce_SEO`, REST `GET /mcpwp/v1/seo/woocommerce`, and MCP `wp_get_woocommerce_seo_report`. It is read-only and checks product description depth, short description coverage, category evidence, product image presence, SKU/price/stock signals, imported search performance, and approval-safe next steps. Commerce mutations remain outside this report.
