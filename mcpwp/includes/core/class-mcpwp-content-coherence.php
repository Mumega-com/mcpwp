...
class Mcpwp_Content_Coherence {
	...
	public static function get_report( $args = array() ) {
		$state = class_exists( 'Mcpwp_Site_State' ) ? Mcpwp_Site_State::get_snapshot( $args ) : array();
		$dimensions = array(
	...
	private static function score_context( $state ) {
		...
		return self::dimension(
			...
			$configured ? __( 'Site context is configured.', 'mcpwp' ) : __( 'Site context is missing.', 'mcpwp' ),
			$configured ? '' : __( 'Define audience, brand, design rules, and content boundaries.', 'mcpwp' )
		);
	}
	...
	private static function score_graph( $state ) {
		...
		return self::dimension(
			...
			sprintf(
				...
				_n( '%d orphan page found.', '%d orphan pages found.', $orphan_count, 'mcpwp' ),
				$orphan_count
			),
			$orphan_count > 0 ? __( 'Connect orphan pages through menus or contextual internal links.', 'mcpwp' ) : ''
		);
	}
	...
	private static function score_content( $state ) {
		...
		return self::dimension(
			...
			sprintf(
				...
				__( '%1$d thin and %2$d stale content items found.', 'mcpwp' ),
				$thin_count,
				$stale_count
			),
			( $thin_count + $stale_count ) > 0 ? __( 'Improve thin pages and refresh stale search-facing content.', 'mcpwp' ) : ''
		);
	}
	...
	private static function score_seo( $state ) {
		...
		return self::dimension(
			...
			sprintf(
				...
				__( '%1$d SEO errors and %2$d SEO warnings are stored.', 'mcpwp' ),
				$errors,
				$warnings
			),
			( $errors + $warnings ) > 0 ? __( 'Resolve stored SEO issues by severity before broad content expansion.', 'mcpwp' ) : ''
		);
	}
	...
	private static function score_governance( $state ) {
		...
		return self::dimension(
			...
			sprintf(
				...
				__( '%1$d pending approvals and %2$d high-risk recent events.', 'mcpwp' ),
				$pending,
				$high
			),
			( $pending + $high ) > 0 ? __( 'Clear pending approvals and review high-risk events before new mutations.', 'mcpwp' ) : ''
		);
	}
	...
	private static function recommendations( $dimensions, $state ) {
		...
		if ( empty( $items ) && ! empty( $state['recommended_actions'] ) ) {
			$items[] = array(
				...
				'message'  => __( 'Choose the next low-risk supervised improvement from the site-state recommendations.', 'mcpwp' ),
