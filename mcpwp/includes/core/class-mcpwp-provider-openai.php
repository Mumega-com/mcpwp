...
class Mcpwp_Provider_OpenAI {
	...
	public function generate_image( $prompt, $size = '1024x1024', $quality = 'medium' ) {
		...
		if ( empty( $response['data'][0]['b64_json'] ) ) {
			return new WP_Error( 'openai_no_image', __( 'No image returned from OpenAI.', 'mcpwp' ) );
		}
	...
	public function analyze_image( $image_url, $instruction = 'Describe this image concisely.' ) {
		...
		if ( empty( $response['choices'][0]['message']['content'] ) ) {
			return new WP_Error( 'openai_no_response', __( 'No response from OpenAI vision.', 'mcpwp' ) );
		}
	...
	public function generate_text( $prompt, $max_tokens = 500 ) {
		...
		if ( empty( $response['choices'][0]['message']['content'] ) ) {
			return new WP_Error( 'openai_no_response', __( 'No response from OpenAI.', 'mcpwp' ) );
		}
	...
	public function test_connection() {
		...
		return array(
			'success' => true,
			'message' => __( 'OpenAI API connection successful.', 'mcpwp' ),
		);
	}
