 ...
 */
class Mcpwp_REST_Multilang extends Mcpwp_REST_API {
	 ...
	 * Constructor.
	 *
	 * @param Mcpwp_Multilang $handler Handler instance.
	 */
			...
			array(
				...
				'args'                => array(
					'language' => array(
						'required'    => true,
						'type'        => 'string',
						'description' => __( 'Language code to set.', 'mcpwp' ),
					),
				),
			)
		);
	...
	private function get_translation_args() {
		return array(
			'language' => array(
				'required'    => true,
				'type'        => 'string',
				'description' => __( 'Target language code.', 'mcpwp' ),
			),
			'title'    => array(
				'type'        => 'string',
				'description' => __( 'Translated title.', 'mcpwp' ),
			),
			'content'  => array(
				'type'        => 'string',
				'description' => __( 'Translated content.', 'mcpwp' ),
			),
			'excerpt'  => array(
				'type'        => 'string',
				'description' => __( 'Translated excerpt.', 'mcpwp' ),
			),
			'status'   => array(
				'type'        => 'string',
				'default'     => 'draft',
				'enum'        => array( 'draft', 'publish', 'pending', 'private' ),
				'description' => __( 'Post status for translation.', 'mcpwp' ),
			),
		);
	}
	...
	public function get_languages() {
		if ( ! $this->handler->is_active() ) {
			return rest_ensure_response( array(
				...
				'plugin'    => null,
				'languages' => array(),
				'message'   => __( 'No multilingual plugin detected.', 'mcpwp' ),
			) );
		}
	...
	public function set_current_language( $request ) {
		if ( ! $this->handler->is_active() ) {
			return new WP_Error(
				'no_multilang',
				__( 'No multilingual plugin is active.', 'mcpwp' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! in_array( $language, $languages, true ) ) {
			return new WP_Error(
				'invalid_language',
				__( 'Invalid language code.', 'mcpwp' ),
				array( 'status' => 400 )
			);
		}
		...
		return rest_ensure_response( array(
			...
			'message'          => sprintf( __( 'Language set to %s.', 'mcpwp' ), $language ),
		) );
	}
