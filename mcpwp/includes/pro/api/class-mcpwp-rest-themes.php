 ...
 */
class Mcpwp_REST_Themes extends Mcpwp_REST_API {
	 ...
	 * Constructor.
	 *
	 * @param Mcpwp_Themes $themes Themes handler.
	 */
	...
	public function get_settings_category( $request ) {
		...
		if ( ! isset( $settings[ $category ] ) ) {
			return $this->error_response(
				...
				sprintf( __( 'Settings category "%s" not found.', 'mcpwp' ), $category ),
				404
			);
		}
	...
	private function check_astra() {
		...
		if ( 'astra' !== $theme_info['slug'] ) {
			return new WP_Error(
				'not_astra',
				__( 'This endpoint requires Astra theme to be active.', 'mcpwp' )
			);
		}
	...
	public function update_astra_colors( $request ) {
		...
		if ( empty( $colors ) ) {
			return $this->error_response(
				'missing_colors',
				__( 'Color data is required.', 'mcpwp' ),
				400
			);
		}
	...
	public function update_astra_typography( $request ) {
		...
		if ( empty( $typography ) ) {
			return $this->error_response(
				'missing_typography',
				__( 'Typography data is required.', 'mcpwp' ),
				400
			);
		}
