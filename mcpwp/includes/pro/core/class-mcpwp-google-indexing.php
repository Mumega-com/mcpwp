...
class Mcpwp_Google_Indexing {
	...
	private function create_jwt( $service_account ) {
		...
		if ( false === $private_key ) {
			return new WP_Error(
				'invalid_private_key',
				__( 'Failed to parse the service account private key.', 'mcpwp' ),
				array( 'status' => 500 )
			);
		}
		...
		if ( ! $success ) {
			return new WP_Error(
				'signing_failed',
				__( 'Failed to sign the JWT.', 'mcpwp' ),
				array( 'status' => 500 )
			);
		}
	...
	private function get_access_token() {
		...
		if ( ! $sa ) {
			return new WP_Error(
				'no_service_account',
				__( 'Google Indexing API service account not configured. Add it via MCPWP > Integrations.', 'mcpwp' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Failed to get access token: %s', 'mcpwp' ),
		...
		if ( 200 !== $code || empty( $body['access_token'] ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: error message */
					__( 'Google OAuth2 error: %s', 'mcpwp' ),
					$error_msg
				),
				array( 'status' => 502 )
			);
	...
	public function submit_urls( $urls, $action = 'URL_UPDATED' ) {
		...
		if ( ! in_array( $action, $valid_actions, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Invalid action. Must be one of: %s', 'mcpwp' ),
		...
		if ( count( $urls ) > 200 ) {
			return new WP_Error(
				'too_many_urls',
				__( 'Maximum 200 URLs per request (Google Indexing API daily quota).', 'mcpwp' ),
				array( 'status' => 400 )
			);
		}
	...
	public function test_connection() {
		...
		return array(
			'success' => true,
			'message' => __( 'Successfully authenticated with Google Indexing API.', 'mcpwp' ),
		);
	}
}
